...
<div class="panel panel-default">
		...
		<tbody class="hide-cart">
			<c:forEach var="p" items="${cart.itemsCart}">
				<tr data-id="${p.id}" data-price="${p.unitPrice}"
					...
					<td>${p.id}</td>
					<td>${p.name}</td>
					<td><f:formatNumber value="${p.unitPrice}" pattern="#,### VND" /></td>
					<td><f:formatNumber value="${p.discount}" type="percent"/></td>
					<td><input value="1" type="number" min="1" max="${p.quantity}"
						class="txt-cart-quantity" style="width: 100px "></td>
					<td class="item-amt"><f:formatNumber value="${p.unitPrice*p.quantity*(1-p.discount)}" pattern="#,### VND" /></td>
					<td>
	...
	<div class="panel-footer">
		<a href="/home/index" class="btn btn-primary">Add More</a>
